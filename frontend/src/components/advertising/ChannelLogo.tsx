...
/**
 * Official platform marks, inline.
 *
 * They were files under public/ads/ and drew as empty tiles for anyone running
 * an ad blocker: the default filter lists drop any request whose path contains
 * "/ads/", which is exactly where the logos of two ad platforms would live.
 * Inline SVG is not a request, so it cannot be blocked - and at ~2.5 kB for the
 * pair it costs less than the two round trips it replaces.
 *
 * Both are the vendors' own marks, unmodified, used to identify the platform.
 */

function GoogleAdsMark({ className }: { className?: string }) {
    return (
        <svg viewBox="0 0 256 230" className={className} aria-hidden="true" focusable="false">
...
function MetaMark({ className }: { className?: string }) {
    // The two gradients are referenced by id, and this mark is drawn several
    // times on the page - a fixed id would make every copy after the first
    // point at the first one's defs.
    const uid = useId().replace(/:/g, '');
    const a = `meta-a-${uid}`;
    const b = `meta-b-${uid}`;
    return (
        <svg viewBox="0 0 256 171" className={className} aria-hidden="true" focusable="false">
            <defs>
...
const MARKS: Record<AdChannel, (props: { className?: string }) => React.ReactElement> = {
    google: GoogleAdsMark,
    meta: MetaMark,
};
...
/**
 * Google Ads / Meta logo. Decorative by default (no label) because it is almost
...
export function ChannelLogo({ channel, size = 'sm', decorative = true, className }: {
    ...
    const mark = <Mark className={cn(SIZE[size], 'shrink-0', className)} />;
    if (decorative) return mark;
    return (
        <span role="img" aria-label={CHANNEL_LABEL[channel]} className="inline-flex">
            {mark}
        </span>
    );
}
