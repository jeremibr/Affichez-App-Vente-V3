...
import { DEPARTMENTS, OFFICES, INVOICE_STATUSES } from '../lib/constants';
...
export default function FWeeklyDetail() {
    ...
    // The rep used in API calls: admin can select, members are locked to their own
    ...
    }, [year, selectedOffice, selectedStatus, repParam, selectedWeek]);

    const fetchWeekData = useCallback(async (weekStart: string, showLoader = true) => {
        if (showLoader) setLoading(true);
        let summaryQuery = supabase.from('v_inv_weekly_summary').select('*').eq('week_start', weekStart);
        ...
        if (repParam) summaryQuery = summaryQuery.eq('rep_name', repParam);

        const [{ data: sData }, { data: lData }] = await Promise.all([
            summaryQuery,
            supabase.rpc('get_inv_weekly_detail', {
        ...
        setLineItems(lData || []);
        if (showLoader) setLoading(false);
    }, [selectedOffice, selectedStatus, repParam]);
    ...
    const filteredSummaryData = useMemo(() => summaryData.filter(row => {
        ...
        const matchRep = !isAdmin || selectedRep === 'Tous' || row.rep_name === selectedRep;
        return matchDept && matchRep;
    }), [summaryData, selectedDept, selectedRep, isAdmin]);

    const filteredLineItems = useMemo(() => lineItems.filter(row => {
        const matchDept = selectedDept === 'Toutes' || row.department === selectedDept;
        const matchRep = !isAdmin || selectedRep === 'Tous' || row.rep_name === selectedRep;
        return matchDept && matchRep;
    }), [lineItems, selectedDept, selectedRep, isAdmin]);

    const filteredGrandTotal = useMemo(() =>
        filteredSummaryData.reduce((sum, row) => sum + Number(row.total_amount), 0), [filteredSummaryData]);

    const avgTicket = useMemo(() =>
        filteredLineItems.length > 0 ? filteredGrandTotal / filteredLineItems.length : 0, [filteredGrandTotal, filteredLineItems]);
        ...
        <div className="p-4 md:p-8 max-w-screen-2xl mx-auto space-y-6 md:space-y-8">
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-20 gap-3">
                    ...
                    <p className="text-sm text-slate-400 font-medium">Récupération des factures...</p>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-card p-20 text-center">
                    ...
                    <h3 className="text-base font-bold text-slate-700">Aucune facture enregistrée</h3>
                    <p className="text-sm text-slate-400 mt-1">Essayez de modifier l'année ou les filtres.</p>
                </div>
            ) : (
                <div className="space-y-6">
                    ...
                    <ZoneBTable lineItems={filteredLineItems} module="factures" />
                </div>
            )}
        </div>
    );
